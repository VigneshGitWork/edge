...
import { variationCodes } from "../utils/constants/common";

class Initializer {
	constructor() {
		console.log("INITILIZER LOADED");
		var self = this;
		this.selectors = {
	...
	initComponent(el) {
		...
		import(
			...
			`../${pathExtension}components/dynamic/${componentName}/${componentName}.js`
		)
			.then((component) => {
			...
			.catch(() => {
				console.debug(
					`../${pathExtension}components/dynamic/${componentName}/${componentName}.js not found`
				);
			});

		import(
			/* webpackExclude: /\.stories\.js$/ */
			`../${pathExtension}components/static/${componentName}/${componentName}.js`
		)
			.then((component) => {
				component.default.init(el);
			})
			.catch(() => {
				console.debug(
					`../${pathExtension}components/static/${componentName}/${componentName}.js not found`
				);
			});
	}
	...
	onDocumentReady() {
		...
		if (pageType === variationCodes.HOME) {
			console.log("%c DOM Loaded", "color: #f48225");
		}
		try {
			...
			this.initMutation();
			if (pageType === variationCodes.HOME) {
				console.log("%c MUTATION INITIALLIZED", "color: #f48225");
			}
		} catch (e) {
			console.log("#####ERROR#######", e);
		}
